...
exports.deleteVarshikMulya = (body) => {
	Upkar.findOne({ id: body.id }, (err, upkar) => { 
...
exports.addVeejKar = (body) => {
	Upkar.findOneAndUpdate({ id: body.id }, { $push: { veej_kar: { name: body.veej_name, rate: body.veej_rate, from_sqft: body.veej_from_sqft, to_sqft: body.veej_to_sqft } } }, (err, up) => {
		if(err) { console.log(err); return handleError(err) };
	})
}
...
exports.addAarogyaKar = (body) => {
	Upkar.findOneAndUpdate({ id: body.id }, { $push: { aarogya_kar: { name: body.aarogya_name, rate: body.aarogya_rate, from_sqft: body.aarogya_from_sqft, to_sqft: body.aarogya_to_sqft } } }, (err, up) => {
		if(err) { console.log(err); return handleError(err) };
	})
}
...
exports.addPanipatti = (body) => {
	Upkar.findOneAndUpdate({ id: body.id }, { $push: { panipatti: { name: body.panipatti_name, rate: body.panipatti_rate } } }, (err, up) => {
		if(err) { console.log(err); return handleError(err) };
	})
}
...
exports.talgharRateUpdate = (body) => {
	Upkar.findOneAndUpdate({ id: body.id }, { $set: { manora_talghar_rate: body.manora_rate } }, (err, up) => {
		if(err) { console.log(err); return handleError(err) };
	})
}

exports.khulliRateUpdate = (body) => {
	Upkar.findOneAndUpdate({ id: body.id }, { $set: { manora_khulli_jaga_normal_rate: body.manora_khulla_rate } }, (err, up) => {
		if(err) { console.log(err); return handleError(err) };
	})
}

exports.imaratRateUpdate = body => {
	Upkar.findOneAndUpdate({ id: body.id }, { $set: { imarat: { home_rate: body.imarat_home_rate, sadnika: body.imarat_sadnika_rate, dukan: body.imarat_dukan_rate, etar: body.imarat_itar_rate } } }, (err, up) => {
		if(err) { console.log(err); return handleError(err) };
	})
}
